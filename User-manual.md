# User Manual: `pgsc_benchmark`

PGS Catalog Benchmarking Analysis

Version 1.1 (Docker/Singularity)

## Software requirements

The PGS benchmarking tool is provided as both a Docker and Singularity image. These
will run best on machines with AMD64 architecture. Please follow the installation
instructions for either [Docker](<https://docs.docker.com/engine/install/>) or
[Singularity](<https://docs.sylabs.io/guides/3.0/user-guide/installation.html>) if
these are not already installed on your research environment.

Additional software is required to run the PGS Catalog Calculator (e.g. Nextflow). Please
see the [online calculator documentation](<https://pgsc-calc.readthedocs.io/en/latest/>)
for details.
...
<table><tr><td><strong>Note:</strong> the files produced by the PGS Catalog Calculator
  ...
  tool, particularly for phenotypes with larger numbers of scores. E.g. we required
...
To run the benchmarking tool, Docker or Singularity should be used to run the relevant image. This
will create the container where the application will execute. You will need to specify all input/output
paths, running requirements and other options when running the image. The following command structure
should be used for Docker and Singularity, respectively:

<code>docker run <em>&lt;input/output binds&gt;</em> pgsc_benchmark <em>&lt;other run options&gt;</em></code>

<code>singularity run <em>&lt;input/output binds&gt;</em> pgsc_benchmark.sif <em>&lt;other run options&gt;</em></code>

### Binding input and output

Containers cannot access files or directories in the host environment after they have been created. To
overcome this, you must bind the input files and desired output directory when calling Docker/Singularity.
This will make the input data available within the container and allow the output to be saved to the host
environment.

To bind a file or directory, specify its path on the host, followed by a colon, then the destination within
the container. Use the <code>&#8209;v</code> option for Docker, or the <code>&#8209;&#8209;bind</code> option
for Singularity:

`-v /path/to/file/on/host:/destination/in/container`

`--bind /path/to/file/on/host:/destination/in/container`

You must bind the input files and output directory to the following paths inside the container:
...
<table>
  ...
  <tbody>
    <tr>
      <td>/app/dem</td>
      <td>Specify the path to the file containing demographic/phenotype data for each participant.
        E.g.<br>~/ukb/bmi.csv:/app/dem</td>
    </tr>
    <tr>
      <td>/app/pgs</td>
      <td>Specify the path to the PGS data generated by the PGS Catalog Calculator (compressed or
        uncompressed). E.g.<br>~/.../score/ukb_pgs.txt.gz:/app/pgs</td>
    </tr>
    <tr>
      <td>/app/pop</td>
      <td>Specify the path to the population similarity data generated by the PGS Catalog Calculator
        (compressed or uncompressed). E.g.<br>~/.../score/ukb_popsimilarity.txt.gz:/app/pop</td>
    </tr>
    <tr>
      <td>/app/out</td>
      <td>Specify the path to where the results directory should be created. You should use the same
        directory across all runs, cohorts, phenotypes, ancestries, etc.
        E.g.<br>~/projects/results/:/app/out</td>
    </tr>
  </tbody>
</table>
...
All other parameters, including data descriptions, relevant column names and run specifications,
must be specified as options after the image name. Most options take only a single argument and
can be specified as follows (no whitespace):

`--option=arg`

Some options (<code>&#8209;&#8209;covs&#8209;cat</code>, <code>&#8209;&#8209;covs&#8209;quant</code>,
<code>&#8209;&#8209;ancestry</code> and <code>&#8209;&#8209;score&#8209;type</code>) can take more
than one argument. Use commas to separate multiple arguments (no whitespace):

`--option=arg1,arg2,arg3`

A full list of available options is provided in <em>Options</em> below.

#### Binary trait example (Docker)

The following example code shows how to run the benchmarking tool for a binary trait
...
```
docker run -d \
    -v ~/projects/results/:/app/out \
    -v ~/UKB/phenotypes/stroke_data.csv:/app/dem \
    -v ~/pgs/stroke/UKB/score/UKB_pgs.txt.gz:/app/pgs \
    -v ~/pgs/stroke/UKB/score/UKB_popsimilarity.txt.gz:/app/pop \
    pgsc_benchmark \
    --sampleset=UKB \
    --phenotype=Stroke \
    --any-case=stroke \
...
<table><tr><td><strong>Note:</strong> the <code>-d</code> flag runs Docker in detached
  mode.</td></tr></table>

#### Quantitative trait example (Singularity)

The following example code shows how to run the benchmarking tool for a quantitative
...
```
singularity run \
    --bind ~/projects/results/:/app/out \
    --bind ~/UKB/phenotypes/bmi_data.csv:/app/dem \
    --bind ~/pgs/bmi/UKB/score/UKB_pgs.txt.gz:/app/pgs \
    --bind ~/pgs/bmi/UKB/score/UKB_popsimilarity.txt.gz:/app/pop \
    pgsc_benchmark.sif \
    --sampleset=UKB \
    --phenotype=BMI \
    --trait-val=bmi \
...
subsequent run of the tool, allowing for a full tree structure of results to be
